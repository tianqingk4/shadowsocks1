...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/tianqingk4/shadowsocks1/master/libsodium-1.0.13.tar.gz'
	else
...
install_centos_ssr(){
	...
	ldconfig
	wget -N --no-check-certificate "https://codeload.github.com/tianqingk4/shadowsocks/zip/master"
	mv master master.zip
	unzip master.zip
	rm -rf master.zip
	mv shadowsocks-master shadowsocks
	cd /root/shadowsocks
...
install_node(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/tianqingk4/shadowsocks1/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
